  /*
      ...
      ;(async function () {
        ...
        class Player {
          ...
          async init() {
            ...
            let RunEnd = (e) => {
              if (e.propertyName === 'transform') {
                ...
                $('.aggregate-player').removeEventListener(
                  'transitionend',
                  RunEnd,
                )
              }
            ...
            playClose.onclick = (e) => {
              ...
              $('.aggregate-player').className =
                'aggregate-player a-p-hide a-c-trs'
            }
            ...
            this.onTr()
            $('.aggregate-run-line').style.backgroundColor =
              setting.color || '#333'
            $('.aggregate-progress-block').style.backgroundColor =
              setting.color || 'white'
            if (setting.autoPlay) {
              let res,
                playlist = loadedPlayList.find(
                  (item) => item.name === setting.album,
                )
              if (!playlist)
                return this.notice.show('自动播放失败,歌单查询不到')
              if (playlist.platform) {
                res = await request(
                  domain +
                    '/api/' +
                    playlist.platform +
                    '?mode=parseRankList&a=' +
                    playlist.uuid,
                )
              } else {
                res = JSON.parse(
                  (
                    await request(
                      domain + '/play/playlist?a=find&uuid=' + playlist.uuid,
                    )
                  ).data.playlist,
                )
              }
            ...
            if (title.innerHTML.indexOf('歌单') !== -1) {
              ...
              if (platform) {
                ...
                if (find.songs) {
                  ...
                  res = await request(
                    domain + '/play/playlist?a=find&uuid=' + uid,
                  )
              ...
              let playlist = title.querySelector('span').innerText,
                songs = loadedPlayList.find((item) => item.name === playlist)
                  .songs
              let currentSong = !songs.length
                ? songs
                : songs.find(
                    (item) => +current.getAttribute('index') === item.id,
                  )
          ...
          async setCurrentSong(song) {
            if (!song.url) {
              ...
              return this.notice.show(
                `歌曲解析失败${this.currentSong ? ',正在切换下一首' : ''}`,
              )
            }
            ...
            if (!this.audio) {
              ...
              this.audio.ondurationchange = function () {
                ...
                Time[1].innerText = `${~~(this.duration / 60)}:${
                  second >= 10 ? '' : 0
                }${second}`
              }
              ...
              this.audio.onended = () => {
                ...
                switch (setting.mode) {
                  ...
                  case 1:
                    let random = () => {
                      var curIndex = this.currentPlayList.findIndex(
                          (item) => item === this.currentSong,
                        ),
                        randomIndex = Math.floor(
                          Math.random() * this.currentPlayList.length,
                        )
                      return randomIndex === curIndex
                        ? random(this.currentPlayList.length)
                        : randomIndex
                    }
              ...
              this.audio.ontimeupdate = function () {
                ...
                if (that.state) {
                  ...
                  let parse = `${minutes >= 10 ? '' : 0}${minutes}:${
                    second >= 10 ? '' : 0
                  }${second}`
                  Time[0].innerHTML = parse
                  $('.aggregate-progress-block').style.width = `${
                    100 * percent
                  }%`
                  if (
                    that.currentSong.lyric &&
                    that.lyricState &&
                    that.currentSong.lyric.size > 1
                  ) {
                    ...
                    if (find.offsetTop >= find.offsetHeight * 5)
                      ...
                      $('.aggregate-lyric').style.transform = `translateY(0px)`
                  }
                } else {
              ...
              $('.player-prev', true).forEach((item) => {
                item.onclick = () => {
                  ...
                  if (
                    this.analyser &&
                    $('.aggregate-media').className.indexOf('a-c-hide') ===
                      -1 &&
                    this.state
                  )
                    this.mediaAnimate(
              ...
              $('.player-next', true).forEach((item) => {
                item.onclick = () => {
                  var curIndex = this.currentPlayList.findIndex(
                    ...
                    target = this.currentPlayList[
                      curIndex >= this.currentPlayList.length - 1
                        ? 0
                        : (curIndex += 1)
                    ]
                  this.setCurrentSong(target)
              ...
              if (setting.mediaShow) {
                $('.toggle-media').onclick = (e) => {
                  if (!this.ac) {
                    ...
                    let source = (this.source = this.ac.createMediaElementSource(
                      this.audio,
                    ))
          ...
          async getCurrentLyric() {
            ...
            let res = await request(
              domain + '/api/' + platform + '?mode=lyric&a=' + id,
            )
            if (res.code !== 200)
              return this.notice.show(res.msg || '获取歌词失败')
            switch (platform) {
              case 'netease':
                if (res.nolyric) {
                  return this.toLyricList(
                    (this.currentSong.lyric = new Map([
                      ['00:00', { content: '该歌曲没有歌词', i: 1 }],
                    ])),
                  )
                }
                ...
                lyric = lyric.filter(
                  (item) =>
                    res[item] && res[item].version >= 1 && res[item].lyric,
                )
                if (!lyric.length)
                  return this.toLyricList(
                    (this.currentSong.lyric = new Map([
          ...
          togglePanel(target, flag) {
            ...
            if (target !== 'list')
              eles.list.className = 'aggregate-list a-c-hide'
            if (target !== 'lyric')
              eles.lyric.className = 'aggregate-lyric a-c-hide'
            if (target !== 'media')
              eles.media.className = 'aggregate-media a-c-hide'
            $('.aggregate-center').style.overflow = ''
            if (flag !== undefined) {
              eles[target].className = flag
                ? `aggregate-${target}`
                : `aggregate-${target} a-c-hide`
            } else {
          ...
          bindMouseDrag(playerRun) {
            ...
            const down = (e) => {
                ...
                ;(disx =
                  (e.pageX || e.targetTouches[0].pageX) - playerRun.offsetLeft),
                  (disy =
                    (e.pageY || e.targetTouches[0].pageY) - playerRun.offsetTop)
                this.moveing = true
              },
              move = (e) => {
                ...
                playerRun.style.transform = `translate(${
                  x - playerRun.offsetLeft + 'px'
          ...
          async handlerError() {
            ...
            if (this.currentSong.error) {
              $('.player-next').onclick()
              return this.notice.show(
                this.currentSong.name + '解析失败,自动切换下一首',
              )
            }
          ...
          async refreshOne() {
            switch (this.currentSong.platform) {
              case 'netease':
                let res = await request(
                  domain + '/api/netease?mode=url&a=' + this.currentSong.id,
                )
                if (res.code !== 200) {
        ...
        try {
          var playerInfo = await request(
              domain + '/play/player?a=find&uuid=' + uuid,
            ),
